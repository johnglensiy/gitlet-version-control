...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(presentValue, rate, targetYear)
                * Math.pow((1 - inflationRate/100), numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        float finalSavings = (float) perYear;
        for(int i = 1; i <= numYears(targetYear); i += 1) {
            finalSavings = finalSavings + ((float) perYear * (float) Math.pow(1 + rate / 100, i));
        }
        return finalSavings;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        return totalSavings(perYear, targetYear, rate)
                * Math.pow((1 - inflationRate/100), numYears(targetYear));
    }
